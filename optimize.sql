...
-- 工單 4：首頁「進行中課程」
CREATE INDEX IF NOT EXISTS idx_courses_end_at_start_at ON courses(end_at,start_at)


-- 工單 5：上週開課課程的教練報名統計（思考方向：需新增兩個索引）
CREATE INDEX IF NOT EXISTS idx_courses_start_at ON courses(start_at);
CREATE INDEX IF NOT EXISTS idx_course_bookings_course_id ON course_bookings(courses_id)

-- 加分題（選做）：使用部分索引（partial index）讓工單 2 的索引更小、更有效率
CREATE INDEX idx_course_bookings_active_user
